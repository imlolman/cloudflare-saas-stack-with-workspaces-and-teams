...
function extractAccountDetails(output: string): { name: string; id: string }[] {
	...
	for (const line of lines) {
		const isValidLine = line.trim().startsWith("│ ") && line.trim().endsWith(" │");
...
async function promptForAccountId(accounts: { name: string; id: string }[]): Promise<string> {
	if (accounts.length === 1) {
		if (!accounts[0]) {
			console.error("\x1b[31mNo accounts found. Please run `wrangler login`.\x1b[0m");
			cancel("Operation cancelled.");
			process.exit(1);
		}
		if (!accounts[0].id) {
...
async function createPagesProject() {
	...
	pagesName = await prompt("Enter the name of your cloudflare pages", defualtPagesName);
	pagesProjectSpinner.start("Creating Pages project...");
	const branch = executeCommand("git branch --show-current");
	executeCommand(`wrangler pages project create ${pagesName} --production-branch ${branch}`);
...
async function createBucketR2() {
	...
	bucketR2Name = await prompt("Enter the name of your bucket", defaultBucketName);
...
async function main() {
	try {
		...
		if (whoamiOutput === undefined || typeof whoamiOutput !== "string") {
			console.error("\x1b[31mError running wrangler whoami. Please run `wrangler login`.\x1b[0m");
			cancel("Operation cancelled.");
			process.exit(1);
		}
